...
class Markdown extends React.Component {
  constructor () {
    ...
    this.applyProps = scope => {
      ...
      Object.keys(defaultProps).forEach(key => {
        if (!scope[key]) return
        scope[key].defaultProps = defaultProps[key]
      })
      return scope
    }
  }
  ...
  render() {
    ...
    const components = this.applyProps(mappedScope)
    const remarkReactComponents = Object.assign({}, defaultScope, components, scope)
